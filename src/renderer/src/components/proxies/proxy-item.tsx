import { Button, Card, CardBody } from '@renderer/components/ui'
...
const ProxyItem: React.FC<Props> = (props) => {
  ...
  return (
    <Card
      ...
      fullWidth
      shadow="sm"
      className={`${fixed ? 'bg-secondary/25' : selected ? 'bg-primary text-primary-foreground dark:bg-primary/80' : 'proxy-node-card bg-white dark:bg-content1'} h-full border border-default-200/35 transition-colors`}
      radius="sm"
    >
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-0.5 shrink-0">
                ...
                <Button
                  ...
                  onPress={onDelay}
                  variant="light"
                  className={`${selected ? 'text-primary-foreground data-[hovered=true]:bg-white/20 hover:bg-white/20' : ''} h-8 w-8 min-w-8 p-0 text-xs`}
                >
                  {delayText(delay)}
                </Button>
              </div>
              ...
              <div className="flex items-center gap-0.5 shrink-0">
                ...
                <div className="flex items-center">
                  <Button
                    ...
                    onPress={onDelay}
                    variant="light"
                    className={`${selected ? 'text-primary-foreground data-[hovered=true]:bg-white/20 hover:bg-white/20' : ''} h-full w-8 min-w-8 p-0 text-sm`}
                  >
                    {delayText(delay)}
                  </Button>
                </div>
